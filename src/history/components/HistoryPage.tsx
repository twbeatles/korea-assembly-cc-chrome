...
} from "../model/history-confirm";
import {
  HISTORY_PAGE_SIZE,
  buildTranscriptSearchMessage,
  formatDate,
  getSessionRowCharCount,
  getSessionRowHasNote,
  getSessionRowId,
  getSessionRowStarred,
  getSessionRowStartedAt,
  getSessionRowStatus,
  getSessionRowSubtitleCount,
  getSessionRowTitle,
  isSessionSearchHit,
  type SessionListRow,
} from "../model/session-list-row";
...
export default function HistoryPage() {
  ...
  const [exportTxtWithoutTimestamps, setExportTxtWithoutTimestamps] = useState(
    DEFAULT_EXTENSION_SETTINGS.exportTxtWithoutTimestamps,
  );
  const [reloadKey, setReloadKey] = useState(0);
  const [busyAction, setBusyAction] = useState<string | null>(null);
  const searchMode = globalSearchQuery.trim().length > 0;

  const filteredEntries = useMemo(
    () => filterEntriesByQuery(selectedSession?.entries ?? [], localSearchQuery),
    [localSearchQuery, selectedSession],
  );
  const checkedIdSet = useMemo(() => new Set(checkedIds), [checkedIds]);
  const checkedEntryIdSet = useMemo(() => new Set(checkedEntryIds), [checkedEntryIds]);
  const displayedSessionRows = useMemo<SessionListRow[]>(
    () => (searchMode ? searchResults : pageSessions),
    [pageSessions, searchMode, searchResults],
  );
  const totalListCount = searchMode ? searchTotalCount : totalSessionCount;
  const pageCount = Math.max(1, Math.ceil(totalListCount / HISTORY_PAGE_SIZE));
  ...
  const currentPageSessionsChecked =
    ...
    displayedSessionRows.every((session) => checkedIdSet.has(getSessionRowId(session)));
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        if (searchMode) {
          const normalizedGlobalQuery = globalSearchQuery.trim();
          const searchResult = await searchSessionsPage({
          ...
          if (searchResult.page !== searchPage) {
          ...
          const matchedSessionIds = new Set(searchResult.results.map((result) => result.sessionId));
          const preservedSelectedSession = previousSelectedSessionRef.current;
          let nextSelectedId = selectedIdRef.current;
          let nextSelectedSession =
            preservedSelectedSession?.id === nextSelectedId ? preservedSelectedSession : null;

          if (nextSelectedId && matchedSessionIds.has(nextSelectedId)) {
            const loadedSelectedSession = await loadSession(nextSelectedId);
            if (!active) {
              return;
            }
            nextSelectedSession = loadedSelectedSession ?? nextSelectedSession;
          } else if (!nextSelectedId && searchResult.results.length > 0) {
            nextSelectedId = searchResult.results[0].sessionId;
            nextSelectedSession = (await loadSession(nextSelectedId)) ?? null;
            if (!active) {
              return;
            }
            setLocalSearchQuery(normalizedGlobalQuery);
          }

          if (!nextSelectedSession && !nextSelectedId && searchResult.results.length === 0) {
            setSelectedId("");
          ...
          if (!preserveMessage) {
            setMessage(messageOnSuccess ?? buildTranscriptSearchMessage(normalizedGlobalQuery, searchResult.totalCount));
          }
          return;
        }
  ...
  }, [globalSearchQuery, reloadKey, searchMode, searchPage, sessionPage, showStarredOnly]);
  ...
  useEffect(() => {
    ...
    void getSettings()
      .then((settings) => {
        ...
        setExportTxtWithoutTimestamps(nextSettings.exportTxtWithoutTimestamps);
      })
      .catch(() => {
  ...
  const handleToggleCheckAll = (): void => {
    const visibleSessionIds = displayedSessionRows.map((session) => getSessionRowId(session));
    setCheckedIds((current) =>
      currentPageSessionsChecked
        ? current.filter((id) => !visibleSessionIds.includes(id))
        : [...new Set([...current, ...visibleSessionIds])],
    );
  };

  const handleToggleFavorite = async (sessionId: string): Promise<void> => {
    const session = await loadSession(sessionId);
    if (!session) {
      setMessage("선택한 기록을 찾지 못했습니다.");
      requestRefresh(undefined, { preserveMessage: true });
      return;
    }
  ...
  const handleSelectSession = async (
    ...
    try {
      const nextSelectedSession =
        pageSessions.find((session) => session.id === sessionId) ?? (await loadSession(sessionId)) ?? null;
      setSelectedId(sessionId);
      setSelectedSession(nextSelectedSession);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "기록을 불러오지 못했습니다.");
      return;
    }
    if (typeof options?.detailQuery === "string") {
      setLocalSearchQuery(options.detailQuery);
    }
  };
  ...
  const handleGlobalSearchChange = (value: string): void => {
        ...
        <aside className="session-list">
          <div className="search-row">
            <input
              ...
              placeholder="전체 기록에서 자막 검색"
            />
            <span>{searchMode ? `검색 결과 ${searchTotalCount}개` : `전체 ${totalSessionCount}개`}</span>
          </div>
          <div className="session-list-toolbar">
            <span>
              {searchMode ? "검색 결과" : "현재 필터"} {totalListCount}개 / 현재 페이지{" "}
              {displayedSessionRows.length}개 / 선택 {checkedIds.length}개
            </span>
            <div className="session-list-actions">
              <button
                className="secondary"
                onClick={handleToggleCheckAll}
                disabled={actionButtonsDisabled || !displayedSessionRows.length}
              >
          ...
          {displayedSessionRows.length ? (
            displayedSessionRows.map((session) => (
              <div key={getSessionRowId(session)} className="session-item-row">
                <label className="session-check">
                  <input
                    type="checkbox"
                    checked={checkedIdSet.has(getSessionRowId(session))}
                    onChange={() => handleToggleChecked(getSessionRowId(session))}
                    disabled={actionButtonsDisabled}
                    aria-label={`${getSessionRowTitle(session)} 선택`}
                  />
                </label>
                <button
                  type="button"
                  className={`favorite-toggle ${getSessionRowStarred(session) ? "active" : ""}`}
                  onClick={() =>
                    runBusyHistoryAction(
                      `favorite_${getSessionRowId(session)}`,
                      () => handleToggleFavorite(getSessionRowId(session)),
                      "즐겨찾기 저장에 실패했습니다.",
                      getSessionRowStarred(session)
                        ? "즐겨찾기 해제를 저장하고 있습니다."
                        : "즐겨찾기를 저장하고 있습니다.",
                    )
                  }
                  ...
                  aria-label={
                    getSessionRowStarred(session)
                      ? `${getSessionRowTitle(session)} 즐겨찾기 해제`
                      : `${getSessionRowTitle(session)} 즐겨찾기 추가`
                  }
                >
                  {getSessionRowStarred(session) ? "★" : "☆"}
                </button>
                <button
                  className={`session-item ${selectedSession?.id === getSessionRowId(session) ? "active" : ""}`}
                  onClick={() =>
                    void handleSelectSession(getSessionRowId(session), {
                      detailQuery: isSessionSearchHit(session) ? globalSearchQuery.trim() : undefined,
                    })
                  }
                  disabled={actionButtonsDisabled}
                >
                  <strong>{getSessionRowTitle(session)}</strong>
                  <span>{formatDate(getSessionRowStartedAt(session))}</span>
                  <span>
                    {getSessionRowSubtitleCount(session)}문장 / {getSessionRowCharCount(session)}자
                  </span>
                  <small>{getPersistedStatusLabel(getSessionRowStatus(session))}</small>
                  {getSessionRowHasNote(session) ? <small>메모 있음</small> : null}
                  {isSessionSearchHit(session) ? (
                    <>
                      <small>일치 항목 {session.matchedCount}개</small>
                      <small>{session.firstSnippet}</small>
                    </>
                  ) : null}
                </button>
              </div>
          ...
          {totalListCount > HISTORY_PAGE_SIZE ? (
            <div className="session-pagination">
              <button
                ...
                disabled={actionButtonsDisabled || (searchMode ? searchPage : sessionPage) <= 1}
              >
                이전 페이지
              </button>
              <span>
                {searchMode ? searchPage : sessionPage} / {pageCount}
              </span>
              <button
                className="secondary"
                onClick={() =>
                  searchMode
                    ? setSearchPage((current) => Math.min(pageCount, current + 1))
                    : setSessionPage((current) => Math.min(pageCount, current + 1))
                }
                disabled={actionButtonsDisabled || (searchMode ? searchPage : sessionPage) >= pageCount}
              >
                다음 페이지
              </button>
            </div>
            ...
            <>
                  ...
                  <button
                    ...
                    onClick={() =>
                      runBusyHistoryAction(
                        ...
                        () => handleToggleFavorite(selectedSession.id),
                        "즐겨찾기 저장에 실패했습니다.",
              ...
              <div className="copy-row">
                ...
                <button
                  onClick={() =>
                    void handleCopy(
                      buildCopyText(selectedSession.entries, { query: localSearchQuery }),
                      "찾은 내용을 복사했습니다.",
                    )
                  }
                  disabled={actionButtonsDisabled || !filteredEntries.length}
              ...
              <div className="entries">
                {filteredEntries.length ? (
                  ...
                  <div className="empty-card">
                    {localSearchQuery ? "찾는 내용이 없습니다." : "이 기록에 자막이 없습니다."}
                  </div>
                )}
              </div>
            </>
