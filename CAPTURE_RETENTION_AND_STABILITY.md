...
- browser E2E smoke 는 `npm run test:e2e:extension` 로 제공하지만, 실제 국회 사이트 운영 DOM 과 완전히 같은 보장은 아니므로 fixture 보강 여지는 남아 있다. 패널은 production closed Shadow DOM 이며, smoke 는 `data-assembly-e2e="1"` 뒤에만 light DOM 미러와 host command 를 사용한다 (`SECURITY.md`).
- 탭이 숨겨진 동안 local polling / observer 폴링 간격은 기본의 4배로 늘어난다 (`resolvePollingIntervalMs`).
- 세그먼트 롤오버 persist 중 이벤트 버퍼 안전 상한은 2048이며, 128은 진단 기준이다. 드롭 누적은 수집 진단 `segmentRollover` 로 노출된다.
- page-exit 저장 큐 목록은 index 키 + 개별 레코드 조회를 우선한다. 인덱스는 mutate 큐로 직렬화하고, startup replay 전에 고아 레코드를 병합한다.
