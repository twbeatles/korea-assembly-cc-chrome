...
- 단일 세션 export는 하드 용량 제한 없이 시도하되, `message length exceeded`/`invalid data URL`/`large data URL fallback disabled` 계열 실패는 사용자 안내 문구로 매핑합니다
...
- options에서는 auto segmentation threshold(`세그먼트 최대 문장/글자/시간`)도 조정할 수 있습니다
...
7. 실행 중 세션이 현재 threshold(`기본값: 2000문장 / 120000자 / 90분`, options에서 조정 가능)를 넘기면, 현재 구간을 저장한 뒤 같은 lineage의 다음 `세그먼트`로 자동 전환한다
...
12. history에서는 세션별 `즐겨찾기`, `메모 저장`, entry 체크박스 기반 `선택한 항목 복사`, `선택 TXT/SRT/VTT/JSON` export를 사용할 수 있고, segmented record 는 `세그먼트 N` 표시와 함께 `연속 캡처 전체 보기` / lineage export 를 지원한다
...
14. 확장 아이콘 popup은 `페이지 패널 열기`, `저장된 기록`, `환경 설정`, `수집 진단`을 빠르게 여는 보조 화면으로 사용하며, 상세 진단은 options 페이지의 `수집 진단` 탭과 `저장 복구 상태` 섹션에서 확인한다. 진단 화면에서는 `persistabilityState` / `persistabilityHint`로 preview-only, unstable-only, filtered, duplicate 상태를 구분할 수 있고, 현재 세그먼트의 threshold 사용량과 포맷별 예상 export 크기도 함께 본다
...
- 수집 진단에는 `persistabilityState` / `persistabilityHint`도 포함되며, panel/popup/options가 같은 이유 문구로 preview-only / unstable-only / filtered / duplicate 상태를 공유합니다
...
- 세그먼트 최대 문장/글자/시간 threshold 도 settings로 저장되며, running capture roll-over 판정에 바로 반영됩니다
...
- 자동 segmentation 이 켜진 상태에서는 세션 레코드에 `lineageId`, `segmentNumber` 메타데이터가 함께 저장되며, history 에서는 이를 `세그먼트 N`, lineage segment 전환, `연속 캡처 전체 보기`로 활용합니다
...
- 매우 큰 export는 Blob 경로를 우선 사용하며, data URL fallback이 비현실적인 크기에서는 범위 분할을 안내하는 명시적 오류로 중단합니다
