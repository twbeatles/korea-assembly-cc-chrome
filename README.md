...
- 단일 세션 export 페이로드가 `8 MiB`를 넘기면 다운로드 요청 전에 사용자 확인을 받습니다. 실패하면 history `저장된 기록` 화면에서 항목을 선택해 부분 export를 사용해 주세요.
- `자막 모으기`를 시작하면 페이지의 `AI 자막보기` 레이어를 자동으로 켜려 시도합니다. 실패하면 패널에 수동 클릭 안내를 표시하며, 강제로 인라인 스타일을 변경하지는 않습니다.
- 자동 시작은 같은 탭에서 같은 player URL에 대해 사용자가 명시적으로 `멈추기`/`화면 비우기`를 했을 경우 새로고침에서도 자동 시작하지 않습니다(다른 player URL이나 새 탭은 영향 없음).
- 옵션의 `수집 중 자동 저장`을 끄면 일반 visibility hidden 시점에는 자동 저장이 일어나지 않습니다. 페이지 닫기 / 탭 종료 직전 stopped 스냅샷은 사양상 항상 저장됩니다.
- 세션 메모(`note`)는 최대 `4096`자까지 저장됩니다.
