...
사용자가 직접 요청한 자막 기록을 TXT, SRT, VTT, JSON, Markdown, CSV 파일로 저장하기 위해 사용합니다. 자동 다운로드는 하지 않으며, 페이지 패널이나 history 화면에서 저장 버튼을 눌렀을 때만 동작합니다. 수동 저장 / 내보내기에서는 확정된 `수집된 자막` 누적 목록만 직렬화하며, preview-only `실시간 내용`은 저장 대상으로 승격하지 않습니다. 단일 세션 export, 선택/중요/시간 범위 export, 전체 기록 JSON 백업도 모두 사용자의 명시적 요청일 때만 실행됩니다.
