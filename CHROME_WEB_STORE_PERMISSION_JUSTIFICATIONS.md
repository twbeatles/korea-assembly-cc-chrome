...
페이지를 닫는 순간 확정 자막 스냅샷을 `chrome.storage.local` 복구 큐에 잠시 남겨 두었다가, 다음 시작에서 IndexedDB 로 재생합니다. 긴 회의 세그먼트는 기본 10MB 할당을 넘을 수 있어, 로컬 복구 큐가 잘리지 않도록 `unlimitedStorage` 를 사용합니다. 데이터는 사용자 브라우저 안에만 있으며 외부 서버로 보내지 않습니다.

Chrome Web Store 제출용 간략 문안:

`unlimitedStorage` 권한은 사용자가 의사중계 페이지를 닫을 때 확정된 자막을 브라우저 로컬 복구 큐에 잠깐 보관했다가 다음 실행에서 되살리기 위해 사용합니다. 긴 회의 기록이 기본 저장 한도를 넘어도 잃지 않게 하려는 것이며, 외부 전송이나 다른 사이트 접근에는 사용하지 않습니다.
