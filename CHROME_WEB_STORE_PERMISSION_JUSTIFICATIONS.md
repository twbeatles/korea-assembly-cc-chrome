...
사용자가 페이지를 닫는 시점에 확정된 자막 세션을 `chrome.storage.local` 복구 큐에 임시 저장하고, 다음 실행 시 IndexedDB에 복원합니다. 긴 회의 기록은 Chrome 기본 저장 한도(약 10MB)를 초과할 수 있으므로 종료 순간의 기록이 잘리거나 유실되지 않도록 `unlimitedStorage`를 사용합니다. 개별 복구 기록은 최대 8MiB로 제한하며, 데이터는 사용자 브라우저 내부에만 저장되고 외부 서버로 전송하거나 다른 웹사이트 데이터에 접근하는 용도로 사용하지 않습니다.

Chrome Web Store 제출용 간략 문안:

`unlimitedStorage` 권한은 사용자가 의사중계 페이지를 닫는 시점에 확정된 자막 세션을 브라우저 로컬 복구 큐에 임시 저장하고, 다음 실행 시 IndexedDB에 복원하기 위해 사용합니다. 긴 회의 기록이 Chrome 기본 저장 한도(약 10MB)를 초과해 종료 순간에 잘리거나 유실되지 않도록 로컬 저장 한도를 완화합니다. 개별 복구 기록은 최대 8MiB로 제한하며, 데이터는 사용자 브라우저 내부에만 저장되고 외부 서버 전송이나 다른 웹사이트 접근에는 사용하지 않습니다.
